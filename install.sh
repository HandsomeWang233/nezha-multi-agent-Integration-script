...
NZ_AGENT_PATH="${NZ_BASE_PATH}/agent-2"
NZ_AGENT_SERVICE_NAME="nezha-agent-2"
NZ_AGENT_SERVICE="/etc/systemd/system/${NZ_AGENT_SERVICE_NAME}.service"
...
pre_check() {
    ...
    if [[ -n "${CUSTOM_MIRROR}" ]]; then
        ...
        if [[ -z "${CN}" ]]; then
            ...
            GITHUB_RAW_URL="gitee.com/naibahq/nezha/raw/master"
            GITHUB_URL="ghfast.top/github.com"
            Get_Docker_URL="get.docker.com"
            Get_Docker_Argu=" -s docker --mirror Aliyun"
            Docker_IMG="registry.cn-shanghai.aliyuncs.com\/naibahq\/nezha-dashboard"
        fi
...
install_arch() {
    ...
    case $input in
    [yY][eE][sS] | [yY])
        useradd -m $NZ_AGENT_SERVICE_NAME
        sed -i "$ a\\$NZ_AGENT_SERVICE_NAME ALL=(ALL ) NOPASSWD:ALL" /etc/sudoers
        sudo -iu $NZ_AGENT_SERVICE_NAME bash -c 'gpg --keyserver keys.gnupg.net --recv-keys BE22091E3EF62275;
        ...
        sed -i '/$NZ_AGENT_SERVICE_NAME/d' /etc/sudoers && sleep 30s && killall -u $NZ_AGENT_SERVICE_NAME && userdel $NZ_AGENT_SERVICE_NAME
...
install_agent() {
    ...
    echo -e "正在下载监控端"
    wget -t 2 -T 10 -O nezha-agent_linux_${os_arch}.zip https://${GITHUB_URL}/HandsomeWang233/nezha-multi-agent-Integration-script/releases/download/v0.20.5/nezha-agent_linux_${os_arch}.zip >/dev/null 2>&1
    if [[ $? != 0 ]]; then
...
modify_agent_config() {
    ...
    if [ "$os_alpine" != 1 ]; then
        wget -t 2 -T 10 -O $NZ_AGENT_SERVICE https://raw.githubusercontent.com/HandsomeWang233/nezha-multi-agent-Integration-script/main/nezha-agent.service >/dev/null 2>&1
        if [[ $? != 0 ]]; then
            echo -e "${red}文件下载失败，请检查本机能否连接 ${GITHUB_RAW_URL}${plain}"
            return 0
        fi
    else
        wget -t 2 -T 10 -O $NZ_AGENT_SERVICERC https://raw.githubusercontent.com/HandsomeWang233/nezha-multi-agent-Integration-script/main/nezha-agent >/dev/null 2>&1
    ...
    if [ "$os_alpine" != 1 ]; then
        sed -i "s#nz_agent_path#${NZ_AGENT_PATH}#" ${NZ_AGENT_SERVICE}
        ...
        [ -n "${args}" ] && sed -i "s|ExecStart=|ExecStart=${args}|" ${NZ_AGENT_SERVICE}
    else
        sed -i "s#nz_agent_path#${NZ_AGENT_PATH}#" ${NZ_AGENT_SERVICERC}
        sed -i "s/nz_agent_name/${NZ_AGENT_SERVICE_NAME}/" ${NZ_AGENT_SERVICERC}
    ...
    if [ "$os_alpine" != 1 ]; then
        ...
        systemctl enable $NZ_AGENT_SERVICE_NAME
        systemctl restart $NZ_AGENT_SERVICE_NAME
    else
        rc-update $NZ_AGENT_SERVICE_NAME
        rc-service $NZ_AGENT_SERVICE_NAME restart
    fi
...
show_agent_log() {
    ...
    if [ "$os_alpine" != 1 ]; then
        journalctl -xf -u $NZ_AGENT_SERVICE_NAME
    else
        echo "很抱歉，OpenRC 无此功能"
    fi
...
uninstall_agent() {
    ...
    if [ "$os_alpine" != 1 ]; then
        systemctl disable $NZ_AGENT_SERVICE_NAME
        systemctl stop $NZ_AGENT_SERVICE_NAME
